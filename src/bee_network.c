...
void beexll_stream_close(php_stream *stream, const char *pid) {
	TSRMLS_FETCH();
...
int beexll_stream_fpid2(const char *pid, php_stream **ostream) {
	TSRMLS_FETCH();
	return php_stream_from_persistent_id(pid, ostream TSRMLS_CC);
}

int beexll_stream_fpid(const char *host, int port, const char *pid,
	...
	*ostream = NULL;
	...
	if (rv == PHP_STREAM_PERSISTENT_NOT_EXIST) {
		return beexll_stream_open(host, port, pid, ostream, err);
	}
	return 0;
}

int beexll_stream_open(const char *host, int port, const char *pid,
		      php_stream **ostream, char **err) {
	TSRMLS_FETCH();
	...
	/* Set READ_TIMEOUT */
	double_to_tv(BEE_G(request_timeout), &tv);
...
error:
	...
	if (stream) beexll_stream_close(stream, pid);
	return -1;
}
 ...
 */
int beexll_stream_read2(php_stream *stream, char *buf, size_t size) {
	TSRMLS_FETCH();
	size_t total_size = 0;
	size_t read_size = 0;
...
int beexll_stream_read(php_stream *stream, char *buf, size_t size) {
	TSRMLS_FETCH();
	return php_stream_read(stream, buf, size);
}

int beexll_stream_send(php_stream *stream, char *buf, size_t size) {
	TSRMLS_FETCH();
