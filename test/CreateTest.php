...
class CreateTest extends PHPUnit_Framework_TestCase
{
		...
		public static function setUpBeforeClass() {
			...
			self::$tm = ini_get("bee.timeout");
			ini_set("bee.timeout", "0.1");
		}

		public static function tearDownAfterClass() {
			ini_set("bee.timeout", self::$tm);
		}

		public function test_00_create_basic() {
			$c = new Bee('localhost', self::$port);
			$c->connect();
		...
		public function test_03_00_create_error_port() {
			(new Bee('localhost', 65500))->connect();
		}
		...
		public function test_03_01_create_error_port() {
			new Bee('localhost', 123456);
		}

		public function test_04_create_many_conns() {
			$a = 1;
			while ($a < 10) {
				$this->assertTrue((new Bee('127.0.0.1', self::$port))->ping());
				$a++;
			}
		}
		...
		public function test_05_flush_construct() {
			$c = new Bee('localhost', self::$port, 'test', 'test');
			$this->assertTrue($c->ping());
			$c->select("test");
			$c->flushSchema();
			$c->select("test");
		...
		public function test_08_good_credentials_construct($username, $password = null) {
			if (func_num_args() === 1) {
				$c = new Bee('localhost', self::$port, $username);
			} else {
				$c = new Bee('localhost', self::$port, $username, $password);
			}
			$this->assertTrue($c->ping());
		}
