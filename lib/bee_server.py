...
class BeeServer(object):
    ...
    def find_exe(self):
        if 'BEE_DB_PATH' in os.environ:
            os.environ["PATH"] = os.environ["BEE_DB_PATH"] + os.pathsep + os.environ["PATH"]
    ...
    def wait_until_started(self):
        ...
        while True:
            try:
                ...
                ans = temp('db.info.status')[0]
                if ans in ('running', 'primary', 'hot_standby', 'orphan', 'loading') or ans.startswith('replica'):
                    return True
                else:
                    raise Exception("Strange output for `db.info.status`: %s" % (ans))
            except socket.error as e:
    ...
    def start(self):
        ...
        #   start Bee\Db       --DONE(prepare_args)
        # * Wait unitl Bee\Db
